...
1.  **安装依赖**:
    ...
    > **环境要求**：需要本机安装 **Chrome 或 Edge** 浏览器（任意版本，默认路径会自动探测）。
    > 站点 `fdc.zjj.sz.gov.cn` 使用了「知道创宇 JSL」反爬虫防护：直接 HTTPS 调用会被拦截（412/400），
    > 只有真实浏览器内由 JSL 脚本动态生成的 `cuyGLa6e` 请求签名才能通过。
    > 项目已内置自动降级：优先直连，被拦截时自动启动真实 Chrome 完成验证后再取数
    > （运行时会短暂弹出浏览器窗口，属正常现象）。若浏览器不在默认路径，可设置环境变量 `CHROME_PATH`。
