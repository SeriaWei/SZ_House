...
const API_URL = 'https://fdc.zjj.sz.gov.cn/api/marketInfoShow/getEsfCjxxGsDataNew';

// Direct HTTPS call (blocked by the JSL anti-bot challenge -> 412/400).
async function fetchSecondHandHomesDirect() {
  const options = {
    method: 'POST',
    headers: {
...
async function getSecondHandHomesData() {
  let data;
  try {
    data = await fetchSecondHandHomesDirect();
  } catch (error) {
